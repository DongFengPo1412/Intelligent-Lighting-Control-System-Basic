...
- **オーディオパワーアンプ (SPK / 8002A)**：車載・産業用小型AB級オーディオアンプ **8002A**（SOP-8）を採用し、5V 駆動、$3\,\Omega$ 負荷時に最大 3W（歪み率 10% 未満）の十分な音量出力を確保；
...
- **電力完全性とIRドロップ対策**：256個のRGB LEDが最大輝度白点灯（$R=G=B=255$）した際の理論ピーク電流は：
  ...
  配線抵抗による電圧降下と色ズレを防止するため、ファームウェア側で全体輝度を 20% ~ 30%（定常平均負荷 1.5A 未満）に制限し、電源母線入力部に $1000\,\mu\text{F}$ の低ESR電解コンデンサを並列接続。
...
- **リセットパルス期間**：ローレベル保持時間 $T_{\text{reset}} > 50\,\mu\text{s}$（ファームウェア標準：$280\,\mu\text{s}$）。
...
| **フルカラーLEDマトリクス** | WS2812B-V5 スマートRGB LED | $8 \times 8$ パネル4枚による $16 \times 16$ 構成（256個）、1677万色、リフレッシュレート > 120Hz |
