...
- **音频功率放大子系统 (SPK / Audio PA)**：选用工业级微型音频功放芯片 **8002A**（SOP-8 封装），在 5V 供电、负载 $3\,\Omega$ 扬声器工况下可输出 3W 额定失真度小于 10% 的充沛音频功率；
...
- **电源完整性设计**：由于 256 颗 RGB LED 满幅白光点亮（全开 $R=G=B=255$）时瞬间峰值工作电流可达：
  ...
  为避免远端供电线压降（IR-Drop）导致尾端色偏与芯片复位，系统在固件中将最大全局亮度钳位在 20% ~ 30%（稳态平均功耗小于 1.5A），并在电源母线输入端并联 $1000\,\mu\text{F}$ 低阻抗铝电解电容，每块子矩阵配备独立加粗的 5V 与 `GND` 供电总线。
