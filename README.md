...
```mermaid
flowchart TD
    A["用户语音指令: 你好小天 / 打开灯光"] -->|差分拾音| B["驻极体拾音麦克风 + 前置放大电路"]
    B -->|模拟音频信号| C["ASRPRO 芯片: AEC 回声消除与硬件 NPU"]
...
- **麦克风前端放大电路 (MIC Circuit)**：采用低噪声稳压偏置源 `MICBIAS`（输出纯净参考电压），通过 C8, C9（0.1 μF）与 R3, R4, R7（2.2 kΩ / 10 kΩ）构成差分输入拓扑，滤除电源共模噪声；
- **音频功率放大子系统 (SPK / Audio PA)**：选用工业级微型音频功放芯片 **8002A**（SOP-8 封装），在 5V 供电、负载 3Ω 扬声器工况下可输出 3W 额定失真度小于 10% 的充沛音频功率；
- **回声消除硬件网络 (AEC Network)**：扬声器正极 `SPKL+` 经 C13（100 nF）与分压衰减电阻 R8, R9 回采至主芯片 `MICP_R` 比较引脚，实现本地播放提示音时的声学回声主动抵消，确保播报时仍能精准打断唤醒。

### 3.2 WS2812B 16×16 智能矩阵拼接设计

- **模组拼接与信号拓扑**：由 4 块独立的 8×8 硬板 RGB 矩阵无缝平铺，采用“上左右下”顺序级联；
- **级联走线**：
...
- **电源完整性设计**：由于 256 颗 RGB LED 满幅白光点亮（全开 `R=G=B=255`）时瞬间峰值工作电流可达：
  ...
  为避免远端供电线压降（IR-Drop）导致尾端色偏与芯片复位，系统在固件中将最大全局亮度钳位在 20% ~ 30%（稳态平均功耗小于 1.5A），并在电源母线输入端并联 1000 μF 低阻抗铝电解电容，每块子矩阵配备独立加粗的 5V 与 `GND` 供电总线。

### 3.3 无线蓝牙通信子系统 (HM-10 BLE 4.0)

- **射频规格**：选用基于 TI CC2541 的 HM-10 蓝牙 4.0 低功耗从机模块，工作频段 2.4GHz ISM 频段，调制方式 GFSK；
- **接口连接**：模块的 `TXD` 和 `RXD` 交叉连接至主控板的硬件串口（波特率 9600 bps 或 115200 bps），手机端 App 通过 BLE GATT 协议向特定特征值写入单字节控制指令（如 `'E'` 开启贪吃蛇，`'W'` 切换笑脸，`'R'`/`'G'`/`'B'` 切换纯色）。
...
WS2812B 数据协议基于精准纳秒级单线归零码（Non-Return-to-Zero, NZR）。每个二进制位传输周期为 `T_bit = 1.25 μs ± 150 ns`：
- **逻辑 0 码**：高电平持续时间 `T_0H = 400 ns ± 150 ns`，低电平持续时间 `T_0L = 850 ns ± 150 ns`；
- **逻辑 1 码**：高电平持续时间 `T_1H = 850 ns ± 150 ns`，低电平持续时间 `T_1L = 400 ns ± 150 ns`；
- **复位帧锁存电平**：低电平持续时间 `T_reset > 50 μs`（固件标准取 `280 μs`）。
...
该帧率远超人眼视觉暂留频率（24Hz），确保了动态表情与彩虹流动光影毫无频闪与撕裂感。
...
1. **预加重与加窗分帧**：采样率 16 kHz，帧长 25 ms，帧移 10 ms，采用汉明窗（Hamming Window）进行时域截断；
...
当综合打分满足置信度门限 $S(\hat{W}) \ge 0.85$ 时，判定识别有效并向主控状态机分发唯一指令 ID。
...
| **离线语音识别能力** | 片上轻量级声学模型 (NPU) | 支持多达 150 条离线本地词条，安静环境识别率 ≥ 98%，响应延迟 < 0.2s |
| **声学前端与扬声器** | 差分驻极体话筒 + 8002A 功放 | 硬件回声消除（AEC），板载 3W 功放芯片，支持 8 级动态数字音量调节 |
| **全彩显示矩阵** | WS2812B-V5 智能 RGB LED | 4 块 8×8 平铺拼接为 16×16（共 256 颗），24-bit 1677 万色，刷新率 > 120Hz |
| **无线通信接口** | HM-10 BLE 4.0 蓝牙从机 | 工作频段 2.4GHz ISM，传输距离空旷处 > 10m，端到端指令通信延迟 < 50ms |
| **外部结构与工艺** | 实木画框 + 亚克力面板 | 尺寸约 150 × 150 × 40 mm，3M 导热胶稳固矩阵，排针镀锡防震走线 |
| **供电系统** | DC 5V 外部稳压电源输入 | 具备防反接与大电容储能滤波，动态工作电流 0.2A ~ 2.0A（亮度自限流保护） |
